...
USStates.Covid19.all %>%
  ...
  ggplot(., aes(x = delta14 , y = rel14, group = USPS)) +
	...
	geom_hline(yintercept = 0)	+	
  #scale_x_log10(limits=c(10,NA), breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x))) +
	scale_x_log10(limits=c(10,NA), breaks = logTicks(n=2),  minor_breaks = logTicks(n=20), labels = trans_format("log10", math_format(10^.x))) +
	annotation_logticks(sides="b") +
	scale_y_continuous(limits=c(-1,3), labels = scales::percent_format(accuracy = 1)) +
	labs(title="New Cases/Capita and Acceleration", subtitle = paste("Covid19 pandemic","-",date()), x="New cases/capita in 2 weeks", y="Relative Increase (%)") +
	theme_bw() }	-> p4
...
USStates.Covid19.all %>%
  ...
  #scale_x_continuous(limits=c(10,NA), trans="log10",breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", math_format(10^.x))) +
	...
	annotation_logticks(sides="b") +
	scale_y_continuous(limits=c(-1,5), trans="identity", labels = scales::percent_format(accuracy = 1)) +
	labs(title="New Cases per Capita and Acceleration", subtitle = paste("Covid19 pandemic","-",date()), x="New cases per Capita in 2 weeks", y="Relative Increase (%)") +
	theme_bw() + 
	transition_time(Date, range=c(date("2020-04-01"),max(USStates.Covid19.all$Date))) +
  ease_aes('linear') +
