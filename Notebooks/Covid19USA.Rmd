...
USStates.Covid19.all %>%
  ...
  geom_smooth(color= 'gray',formula ='y ~ x', method = 'loess', span=0.10, se=FALSE) +
	geom_point(data=filter(., USPS %in% c('MA','NY','CA','FL','TX','AZ')), aes(color=USPS), shape=1) +	
	geom_smooth(data=filter(., USPS %in% c('MA','NY','CA','FL','TX','AZ')), aes(color=USPS), formula ='y ~ x',method = 'loess', span=0.10, se=FALSE) +	
...
####################################################################
if(wday(today(),label=TRUE) == "Tue") { # takes hours, only do this once a week and save
USStates.Covid19.all %>%
	filter(Metric == "Confirmed", Metric2 %in% c('delta14', 'rel14')) %>% 
