...
if (!file.exists(county_path)) {
  ...
  pair_blocking(transmute(USCounties.Census, FIPS, STATE, region=tolower(STNAME), subregion=tolower(CTYNAME)), 
	  						group_by(map.county, STATE, region, subregion) %>% summarise(Long=mean(long), Lat=mean(lat)), 
...
USStates.Covid19 %>%
	...
	select(STATE, val, Lat, Long) %>% 
	{
	left_join(map.state, ., by="STATE") %>% 	
  ggplot(aes(x = long, y = lat)) +
  geom_polygon(aes(group = group, fill=val), colour = "white") +
	geom_text(data=filter(USStates, !STNAME %in% c("Alaska","Hawaii")), aes(label=STNAME, x = Long, y = Lat), size=3, colour = "black", hjust=.5) +
	geom_text(data=., aes(label=ks(val,acc=0.1), x = Long, y = Lat), size=3, colour = "black", vjust=2) +
...
USStates.Covid19 %>%
	...
	select(STATE, val,Long, Lat) %>%
	{
	left_join(map.state, ., by="STATE") %>% 
  ggplot(aes(x = long, y = lat)) +
  geom_polygon(aes(group = group, fill=val), colour = "white") +
	geom_text(data=filter(USStates, !STNAME %in% c("Alaska","Hawaii")), aes(label=STNAME, x = Long, y = Lat), size=3, colour = "green", hjust=.5) +
	geom_text(data=., aes(label=val, x = Long, y = Lat), size=3, colour = "green", vjust=2) +
  geom_point(data=data.frame(lat=42.8126, long=-70.8773), size=4, shape=1, color='red') +
	theme_map() + 
	coord_map() +
...
USStates.Covid19 %>%
  ...
  ggplot(aes(x = long, y = lat)) +
  geom_polygon(aes(group = group, fill=val), colour = "white") +
	geom_text(data=filter(USStates, !STNAME %in% c("Alaska","Hawaii")), aes(label=STNAME, x = Long, y = Lat), size=3, colour = "darkgreen", hjust=.5) +
	geom_text(data=., aes(label=val, x = Long, y = Lat), size=3, colour = "darkgreen", vjust=2) +
  geom_point(data=data.frame(lat=42.8126, long=-70.8773), size=4, shape=1, color='red') +
	theme_map() + 
	coord_map() +
...
AnimateCountyMap <- function (giffile, m1="Confirmed", m2='n', pc=FALSE) {
  ...
  theme_map() + 
	...
	animate(p, width = 1920, height = 1080, fps=10, duration=10, end_pause=5) %>% 
...
knitr::include_graphics("USMap_Confirmed_n_FALSE.gif")
```
...
- annimation of states. cumm confirmed cases vs cumm deaths see https://covid19.who.int/explorer
...
- oad geo state and county areas, population density
