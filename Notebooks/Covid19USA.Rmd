...
USStates.Covid19 %>%
  ...
  ggplot(aes(x = long, y = lat)) +
  geom_polygon(aes(group = group, fill=val), colour = "white") +
	geom_text(data=filter(USStates, !STNAME %in% c("Alaska","Hawaii")), aes(label=STNAME, x = Long, y = Lat), size=3, colour = "gray", hjust=.5) +
  geom_point(data=data.frame(lat=42.8126, long=-70.8773), size=4, shape=1, color='red') +
	theme_map() + 
	coord_map() +
...
### Heat map New Cases per Week 

```{r,fig.width=13}
####################################################################
# Heat map:  New Confirmed Cases per week 
####################################################################

USStates.Covid19.all %>%  
	filter(Metric == "Confirmed", Metric2 == 'delta7') %>% 
	left_join(start, by="STNAME") %>% 
	ggplot(aes(x=Date, y=reorder(STNAME, as.integer(StartDate)))) +
  geom_tile(aes(fill=val)) + 
	scale_x_date(limits = c(date("2020-02-15"), NA)) +
	scale_y_discrete(position = "right")	+	
	labs(title="US State New Confirmed Cases per Week", subtitle = paste("Covid19 pandemic","-",date()), x="Time(2020)", y="") +
	theme_bw() +
	theme(legend.position = "right") +	
	scale_fill_viridis( option = "inferno", direction = +1, name = "Cases", limits= c(0,NA), trans= "pseudo_log", breaks=10^seq(0,5), 
											labels = label_number(accuracy = 1,big.mark = ','),
