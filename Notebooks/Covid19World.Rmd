...
This second table below shows a ranked list of countries with new confirmed cases per capita per week. as in the previous table, tt shows the new cases in the last week from the date on the upper left, but this time per 100k people. It also shows the maximum cases per capita per week and when that peak happened. In addition it shows the relative decrease in number of weekly cases compared to the peak
	...
	mutate(NewCases = n/lag(n)-1 ) %>%
{
  ...
  geom_point(shape=1, size=3, colour='gray') + 
	...
	geom_label_repel(data=filter(., Date==max(Date)), aes(label = paste(`Country/Region`, percent(NewCases, accuracy=0.01),sep=',')), nudge_x=0.7, colour='grey') +
	scale_x_date() +
...
df25 %>% filter(Metric == 'Deaths') %>% 
  ...
  geom_point(shape=1, size=3, colour='gray') + 
	geom_smooth(formula ='y ~ x', method = 'loess', se=FALSE, colour='gray') +
	geom_smooth(data=filter(.,`Country/Region`=='US'),formula ='y ~ x', method = 'loess', se=FALSE) +	
	geom_label_repel(data=filter(., Date==max(Date)), aes(label = paste(`Country/Region`, percent(New, accuracy=0.1),sep=',')), nudge_x=0.7, colour='grey') +
	scale_x_date() +
	scale_y_log10(limits=c(10^-3,1),breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10", percent)) +
